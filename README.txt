...
***** 商用情况 *****
     xdisp_virt整套工程源码不开源，也并不出售。带水印的版本严禁商用。
     软件除了图像会出现可移动的水印版权之外，并未做任何限制。
     程序提供了去除水印的许可证功能，如果商业使用或不想看到水印，请使用许可证。
...
程序开发及开放源代码说明：
    ...
    同时 xdisp_virt 程序远程图像会有水印。
    带水印的版本严禁商用。如有商用需求请购买相应许可证。

2026年1-7月更新：
1，七以上的版本，提供了许可证功能，用以去除水印，但须付费，具体详情请查阅相关说明。
2，linux版本集成了 Wayland，Xorg，FrameBuffer三种桌面图像采集方案，并且每种采集方式都是采用动态加载
      的方式，这意味着：比如即使某个古老系统没有Wayland环境，也不会让程序无法运行；
      或者比如只有FrameBuffer没有Wayland等环境，程序同样能运行。
3，添加了半自动更新功能，也就是如果有新版本，你需要手动从github下载对应平台的xdisp_virt程序到本地，
      然后通过xdisp_virt提供的版本管理页面，上传最新版本程序去更新。
4，优化了windows平台的DXGI采集算法，使得采集性能更优良。
5，各种小功能的添加和优化。

2025年10-12月更新：
1，在windows平台添加了DWM HOOK功能，利用此功能，可使得本地电脑显示器生成变形的桌面图像。
     此为实验性质的功能，不是所有windows平台都支持，具体电脑需要具体去检测验证。
2，目录传输页面添加了二维码扫描下载功能， 以及文本在线编辑功能。
3，图像特效增加了浮雕，摩尔纹，卡通，以及各类色彩特效。采用多线程处理特效算法，一定程度上增加了速度。

2025年07-08月更新：
1，在xdisp_virt远程控制的浏览器客户端添加录制到MP4视频文件功能，
      直接在web浏览器端截获音视频流，并且录制成MP4视频文件，而不是在被控制端录制视频。
2，windows平台的xdisp_virt程序添加了远端虚拟打印机功能。
     在WIN10以上系统可以直接从Microsoft Print to PDF内置驱动克隆虚拟驱动实现远程打印，
     也可以在全windows平台（从WINXP到WIN11）使用xdisp_virt提供的简易 Esay Printer 实现远程打印。
     也可以接入符合条件的第三方虚拟打印机驱动。
     相关技术细节可查阅：
      https://blog.csdn.net/fanxiushu/article/details/151189152?spm=1001.2014.3001.5501

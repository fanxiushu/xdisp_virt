...
WebAudioOut.prototype.setVolume = function (vol) {
    this.volume = vol;
    console.log('VolumeRate='+this.volume );
}
WebAudioOut.prototype.getVolume = function () {
    return this.volume;
}
...
WebAudioOut.prototype.play = function(sampleRate, left, right) {
	...
	for (i = 0; i < left.length; i++) {
	    v = left[i]*this.volume;
	    if (v > 1.0) v = 1.0; else if (v < -1.0) v = -1.0;
	    left[i] = v;
	}
	for (i = 0; i < right.length; i++) {
	    v = right[i] * this.volume;
	    if (v > 1.0) v = 1.0; else if (v < -1.0) v = -1.0;
	    right[i] = v;
	}
    //////

	this.gain.gain.value = 1;//this.volume;
